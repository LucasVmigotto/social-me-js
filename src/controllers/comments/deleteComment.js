...
module.exports = async ({ params, knex, logger, user }, res) => {
  try {
    ...
    const [commentCreator] = await knex('social_me.comments')
      .select('user_id')
      .where('id', commentId)

    const [postOwner] = await knex('social_me.posts')
      .select('posts.user_id')
      .leftJoin('social_me.comments', 'post_id', 'posts.id')
    ...
    if (!deletedBy) {
      ...
      return makeResponse(
        res,
        409,
        { message: 'Only comment creators or related post owners can remove comments' }
      )

    }

    const [commentDeleted] = await knex('social_me.comments')
